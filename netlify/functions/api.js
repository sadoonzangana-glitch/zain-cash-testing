...
1. يُحراء تماماً على أي وكيل مرخص استقطاع أي عمولة إضافية خارج النسب الرسمية المحددة في التطبيق.
...
exports.handler = async (event, context) => {
    ...
    if (cleanPath === '/login' && event.httpMethod === 'POST') {
        ...
        if (!username) return { statusCode: 400, headers, body: JSON.stringify({ error: "Username is required" }) };
        ...
        if (user) {
            ...
            return { statusCode: 401, headers, body: JSON.stringify({ error: "Employee code not registered" }) };
        }
    }

    if (cleanPath === '/users' && event.httpMethod === 'GET') {
        return { statusCode: 200, headers, body: JSON.stringify(db.users || []) };
    }

    if (cleanPath === '/users/update-email' && event.httpMethod === 'POST') {
    ...
    if (cleanPath === '/scenarios') {
        if (event.httpMethod === 'GET') {
            return { statusCode: 200, headers, body: JSON.stringify(db.scenarios) };
        }
        if (event.httpMethod === 'POST') {
            db.scenarios = bodyData;
            await saveDb(db);
            return { statusCode: 200, headers, body: JSON.stringify({ success: true }) };
        }
    }
    ...
    if (cleanPath === '/call-scenarios') {
        if (event.httpMethod === 'GET') {
            return { statusCode: 200, headers, body: JSON.stringify(db.callScenarios || [
                {
                    id: 'inbound-1',
                    type: 'inbound',
    ...
    if (cleanPath === '/kb') {
        if (event.httpMethod === 'GET') {
            return { statusCode: 200, headers, body: JSON.stringify(db.knowledgeBase || defaultKb) };
        }
        if (event.httpMethod === 'POST') {
            db.knowledgeBase = bodyData;
            await saveDb(db);
            return { statusCode: 200, headers, body: JSON.stringify({ success: true }) };
        }
    }
    ...
    if (cleanPath === '/assignments') {
        if (event.httpMethod === 'GET') {
            return { statusCode: 200, headers, body: JSON.stringify(db.assignments || []) };
        }
        if (event.httpMethod === 'POST') {
    ...
    if (cleanPath === '/call-assignments') {
        ...
        if (event.httpMethod === 'POST') {
            ...
            db.callAssignmentsMeta = { assignedAt: Date.now() };
            db.testSessions = db.testSessions || {};
            const targetUserIds = bodyData || [];
    ...
    if (cleanPath === '/results') {
        ...
        if (event.httpMethod === 'POST') {
            const newRes = bodyData;
            if (!newRes.date) {
                newRes.date = new Date().toISOString().replace('T', ' ').substring(0, 19);
            }
    ...
    if (cleanPath === '/ai-results') {
        if (event.httpMethod === 'GET') {
            return { statusCode: 200, headers, body: JSON.stringify(db.aiResults || []) };
        }
        if (event.httpMethod === 'POST') {
            const newRes = bodyData;
            if (!newRes.date) {
